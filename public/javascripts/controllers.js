...
angular.module('ttt').controller('GamesCtrl', [   
  ...
  function($scope, $http, $rootScope, $auth, Account, $state, $timeout, $mdToast, $animate, socketio) {
	...
	$scope.messages=[]; 				//variable qui va contenir la liste des messages communiqués entre les joueurs
	
	$scope.msg = {};					//variable pour le nouveau message saisi
	
	$scope.sendMessage = function(m) {  //fonction appelée lors du click sur le bouton d'envoi du message
										// ou le click sur "Enter" du clavier
	
	//construction de l'objet à envoyer en broadcast pour afficher les nouveaux messages envoyés
	
	 var msgToSent = {
		 username :  $rootScope.user.user_name ,  //récupération du nom de l'utilisateur qui a saisi le message
		 message : m,   						  // msg saisi
		 date : new Date(),						  // date d'envoi du msg (date actuelle)
		 avatar :  $rootScope.user.avatar + '.png' //récupération de l'avatar(image associé à l'utilisateur ) 
													//de l'utilisateur qui a saisi le message
	 };
	 
	  socketio.emit('newMessage',msgToSent);   //envoi de l'objet construit vers les utilisateurs connectés
											// via socketio pour mettre à jour la liste des messages afichée pour chaque utilisateur
											
	  $scope.msg = {};         				//supprimer le message envoyé de la zone de saisie
 }
    ...
    Account.getGame()
      .success(function(data) {
        $rootScope.user = data;
		 $rootScope.user.avatar=Math.floor((Math.random() * 5) + 1);
      })
      .error(function(error) {
        console.log(error);
      });
    ...
    // 2D array board
     ...
     $scope.newGame = function() {
       ...
       socketio.on('PlayerTurnEmit', function(data){
	    ...
	    socketio.on('newMessage',function(message){  //déclaration du traitement à faire lors de la réception d'un événement
													// de type 'newMessage' via socketio
													
         $scope.messages.push(message);			  // ajout du message reçu à la variable  $scope.messages
												// le message sera ajouté automatiquement à la fin de la liste des messages affichés 
		});
